...
def save_to_file(result, start_row, end_row, output_dir, thresh):
    frame = pd.DataFrame(result,
                         columns=["nube1", "tamaño_nube1", "nube2", "tamaño_nube2", "matcheos", "overlap", "label",
                                  "rmse", "radio", "giros"])
    path = output_dir + "/180_vs_freestyle_thresh_0.7_radio_" + str(thresh) + '_rows_' + str(start_row) + '_' + str(end_row) + ".csv"
    print(f'partial saving in {path}')
    frame.to_csv(path)

def main():
    freestyle_input_dir = '/data/freestyle/thresh0.7/' # 'F:/Escritorio/repo_2023/identificaci-nDeRacimos/input/2023.03_captura_2/freestyle/thresh0.7/'
    a180_input_dir =  '/data/180/thresh0.7/' #  'F:/Escritorio/repo_2023/identificaci-nDeRacimos/input/2023.03_captura_2/180/thresh0.7/'
    output_dir = '/data/output/180_vs_freestyle/' #  'F:/Escritorio/repo_2023/identificaci-nDeRacimos/output/2023.03_captura_2/180_vs_freestyle/'
    freestyle_inputs_path = freestyle_input_dir + "labels.csv"
    a180_inputs_path = a180_input_dir + "labels.csv"
    freestyle_inputs_df = pd.read_csv(freestyle_inputs_path)
    ...
    freestyle_clouds = outliers_filter_v2(freestyle_clouds)
    freestyle_clouds = outliers_filter_v2(freestyle_clouds)
    freestyle_clouds = outliers_filter_v2(freestyle_clouds)
    freestyle_clouds = outliers_filter_v2(freestyle_clouds)
    freestyle_clouds = outliers_filter_v2(freestyle_clouds)
    freestyle_clouds = outliers_filter_v2(freestyle_clouds)
    freestyle_clouds = duplicates_filter_v2(freestyle_clouds)
